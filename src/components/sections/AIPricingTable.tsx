'use client'

import { memo } from 'react'
import Link from 'next/link'
import { Check } from 'lucide-react'
import { COMPANY_DATA } from '@/lib/company-data'
import { cn, HEADING, TEXT, MARGIN, GRID, card, CARD_PADDING, button } from '@/lib/design-system/unified'
import UltraSection from '@/components/ui/UltraSection'

const AIPricingTable = memo(function AIPricingTable() {
  const aiPricing = COMPANY_DATA.services.details.ai.pricing

  return (
    <UltraSection variant="dark">
      <div className={cn('text-center', MARGIN.xl)}>
        <h2 className={cn(HEADING.h2, 'text-gray-900', MARGIN.sm)}>
          料金プラン
        </h2>
        <p className={cn(TEXT.lead, 'text-gray-900')}>
          具体的な導入イメージと投資効果を無料で試算できます
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-7 md:gap-8 max-w-7xl mx-auto">
        {/* Basic Plan */}
        <div className={cn(card({ variant: 'default' }), CARD_PADDING.lg)}>
          <h3 className={cn(HEADING.h4, 'text-gray-900', MARGIN.xs)}>
            {aiPricing.basic.name}
          </h3>
          <div className={MARGIN.md}>
            <span className={cn(HEADING.h2, 'text-gray-900')}>
              {aiPricing.basic.price}
            </span>
            <span className={cn(TEXT.body, 'text-gray-700 ml-2')}>
              / {aiPricing.basic.duration}
            </span>
          </div>

          <ul className={cn('space-y-3', MARGIN.lg)}>
            {aiPricing.basic.features.map((feature, index) => (
              <li key={index} className="flex items-start gap-3">
                <Check className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" />
                <span className={cn(TEXT.small, 'text-gray-700')}>{feature}</span>
              </li>
            ))}
          </ul>

          <Link href="/#contact" className={button({ variant: 'primary', size: 'lg', className: 'w-full' })}>
            無料相談を予約する
          </Link>
        </div>

        {/* AI Common Plan - Featured */}
        <div className="relative border-2 border-blue-600 rounded-2xl shadow-xl sm:col-span-2 md:col-span-1">
          <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
            <span className="bg-blue-600 text-white px-6 py-1 rounded-full text-sm font-bold shadow-lg">
              おすすめ - 月額制
            </span>
          </div>
          <div className={cn(card({ variant: 'default' }), CARD_PADDING.lg, 'border-2 border-blue-200')}>
            <h3 className={cn(HEADING.h4, 'text-gray-900', MARGIN.xs)}>
              {aiPricing.aiCommon.name}
            </h3>
            <div className={MARGIN.md}>
              <span className={cn(HEADING.h2, 'text-gray-900')}>
                {aiPricing.aiCommon.price}
              </span>
              <span className={cn(TEXT.body, 'text-gray-700 ml-2')}>
                / {aiPricing.aiCommon.duration}
              </span>
            </div>

            <ul className={cn('space-y-3', MARGIN.lg)}>
              {aiPricing.aiCommon.features.map((feature, index) => (
                <li key={index} className="flex items-start gap-3">
                  <Check className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                  <span className={cn(TEXT.small, 'text-gray-700')}>{feature}</span>
                </li>
              ))}
            </ul>

            <Link href="/#contact" className={button({ variant: 'primary', size: 'lg', className: 'w-full' })}>
              プラン詳細を見る
            </Link>
          </div>
        </div>

        {/* Enterprise Plan */}
        <div className={cn(card({ variant: 'default' }), CARD_PADDING.lg)}>
          <h3 className={cn(HEADING.h4, 'text-gray-900', MARGIN.xs)}>
            {aiPricing.enterprise.name}
          </h3>
          <div className={MARGIN.md}>
            <span className={cn(HEADING.h2, 'text-gray-900')}>
              {aiPricing.enterprise.price}
            </span>
            <span className={cn(TEXT.body, 'text-gray-700 ml-2')}>
              / {aiPricing.enterprise.duration}
            </span>
          </div>

          <ul className={cn('space-y-3', MARGIN.lg)}>
            {aiPricing.enterprise.features.map((feature, index) => (
              <li key={index} className="flex items-start gap-3">
                <Check className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" />
                <span className={cn(TEXT.small, 'text-gray-700')}>{feature}</span>
              </li>
            ))}
          </ul>

          <Link href="/#contact" className={button({ variant: 'outline', size: 'lg', className: 'w-full' })}>
            詳細を問い合わせる
          </Link>
        </div>
      </div>

      {/* AI顧問 会社規模別価格表 */}
      <div className="mt-16 max-w-5xl mx-auto">
        <div className={cn('text-center', MARGIN.lg)}>
          <h3 className={cn(HEADING.h3, 'text-gray-900', MARGIN.sm)}>
            AI顧問 会社規模別料金
          </h3>
          <p className={cn(TEXT.body, 'text-gray-900')}>
            従業員数に応じた最適なプランをご提案
          </p>
        </div>

        <div className={GRID.three}>
          {aiPricing.aiCommon.tiers.map((tier, index) => (
            <div key={index} className={cn(card({ variant: 'default' }), CARD_PADDING.md, 'hover:shadow-xl')}>
              <div className="text-center mb-3 sm:mb-4">
                <h4 className="text-lg sm:text-xl font-bold text-blue-700 mb-2">{tier.size}</h4>
                <p className="text-sm text-gray-600">{tier.employees}</p>
              </div>
              <div className={cn('text-center', MARGIN.md)}>
                <span className={cn(HEADING.h3, 'text-gray-900')}>{tier.price}</span>
              </div>
              <ul className="space-y-2">
                {tier.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-2">
                    <Check className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <span className={cn(TEXT.small, 'text-gray-700')}>{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>
      </div>

      <div className="mt-12 text-center">
        <p className={cn(TEXT.small, 'text-gray-600')}>
          ※ 料金はすべて税込価格です
        </p>
      </div>
    </UltraSection>
  )
})

export default AIPricingTable
